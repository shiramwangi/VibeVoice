...
class VibeVoiceProcessor:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if os.path.exists(config_path):
            ...
            logger.warning(
                f"No preprocessor_config.json found at {pretrained_model_name_or_path}, using defaults"
            )
            config = {
                "speech_tok_compress_ratio": 3200,
                "db_normalize": True,
    ...
    def _convert_text_to_script(self, text_file: str) -> str:
        ...
        for line in lines:
            ...
            speaker_match = re.match(
                r"^Speaker\s+(\d+)\s*:\s*(.*)$", line, re.IGNORECASE
            )
    ...
    def _parse_script(self, script: str) -> List[Tuple[int, str]]:
        ...
        for line in lines:
            ...
            match = re.match(
                r"^Speaker\s+(\d+)\s*:\s*(.*)$", line.strip(), re.IGNORECASE
            )
