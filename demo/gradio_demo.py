...
class VibeVoiceDemo:
    ...
    def load_model(self):
        ...
        # Load model
        # Configure dtype and attention implementation based on device
        use_cpu = (self.device == "cpu") or (not torch.cuda.is_available())
        dtype = torch.float32 if use_cpu else torch.bfloat16
        attn_impl = "eager" if use_cpu else "flash_attention_2"
...
Or paste text directly and it will auto-assign speakers.""",
            ...
            try:
                ...
                for (
                    ...
                    if complete_audio is not None:
                        ...
                        yield (
                            None,
                            gr.update(value=complete_audio, visible=True),
                            log,
                            gr.update(visible=False),
                            gr.update(visible=True),
                            gr.update(visible=False),
        ...
        def load_random_example():
            ...
            if (
                hasattr(demo_instance, "example_scripts")
                and demo_instance.example_scripts
            ):
